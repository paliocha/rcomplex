...
This article collects the statistical and implementation detail behind
rcomplex. The [README](https://github.com/paliocha/rcomplex#readme) gives
a short first analysis; the tutorial vignette runs the full pipeline on
eight Pooideae grasses. The code below is annotated at the level of
individual parameters and is not run when the site is built. It assumes
objects (`expr1`, `net1`, the eight-species `networks`, `modules`,
`edges`) built as in the tutorial.
...
# 1. Parse ortholog groups (tab-delimited, format in the README)
...
# randomized p-values: pass `seed` for reproducible q-values. Directional
# q-values are combined with pval_combine = "max" by default (both
# directions must be significant -- the reciprocal criterion of Netotea
# et al., 2014).
summary <- summarize_comparison(comparison, seed = 1)
...
- **`mr_log_transform = FALSE`** (raw mutual rank, the default): Returns
  `MR(i,j)` directly. Values range from 1 to n. On the Pooideae and wood
  data the two modes changed co-expressolog call counts by 1-9 %.
