...
// partners whose rank lies in the top fraction f of that column.
// Raw MR sqrt(R_ij * R_ji) (ascending ranks, both at most n): a pair at or
// above threshold T has min(R_ij, R_ji) >= T^2 / n, so a pair is a
// candidate when each gene is on the other's list.
// Log MR (descending ranks, v >= T <=> R_ij * R_ji <= n^(2(1 - T))): a
// qualifying pair has min(R_ij, R_ji) <= n^(1 - T), so a pair is a
...
List mr_block_network_cpp(const arma::mat& zt, bool log_transform,
    ...
    // Raw MR of (i, j) from list ranks, as mutual_rank_inplace_cpp()
    // computes it; -1 when the pair is not on both lists.
    std::vector<List_t> lists(n);
    auto mr = [&](int i, const Entry& e) {
        const double rji = find_rank(lists[e.j], i);
        if (rji < 0) return -1.0;
        return std::sqrt(static_cast<double>(e.r) * rji);
    };
    ...
    // Log MR: the reverse index holds, for each gene b (CSR offsets roff),
    // the genes a (in Entry::j, ascending) with R_a(b) <= U, and lv the
    // exact log MR of each such pair (NaN for the second copy of a pair
    // listed both ways).
    std::vector<std::size_t> roff;
    std::unique_ptr<Entry[]> rev;
    std::unique_ptr<double[]> lv;

    auto select = [&](std::vector<double>& cand) {
    ...
    for (;;) {
        ...
        // Pass 1: exact ranks per column, keep the top-f partners.
        sweep([&](R_xlen_t i, const double* col) {
        ...
        if (log_transform) {
            ...
            roff.assign(n + 1, 0);
            for (const List_t& l : lists) {
                for (const Entry& e : l) ++roff[e.j + 1];
            }
            for (R_xlen_t b = 0; b < n; ++b) roff[b + 1] += roff[b];
            rev.reset(new Entry[roff[n]]);
            lv.reset(new double[roff[n]]);
            std::vector<std::size_t> fill(roff.begin(), roff.end() - 1);
            for (int a = 0; a < ni; ++a) {
            ...
            sweep([&](R_xlen_t b, const double* col) {
                for (std::size_t k = roff[b]; k < roff[b + 1]; ++k) {
                    const int a = rev[k].j;
            ...
            n_cand = 0;
            for (std::size_t k = 0; k < roff[n]; ++k) {
                n_cand += !std::isnan(lv[k]);
            }
            if (n_cand >= k_s) {
                ...
                valid = f >= 1.0 ||
                    std::pow(dn, 1.0 - t_s) * (1.0 + 1e-12) <= lim;
            }
        } else {
            // Join: candidate MR values of the upper triangle, counted
            // first.
    ...
    if (log_transform) {
        for (int b = 0; b < ni; ++b) {
            for (std::size_t k = roff[b]; k < roff[b + 1]; ++k) {
                if (!(lv[k] >= t_s)) continue;  // also skips NaN
                const int a = rev[k].j;
                kept[std::min(a, b)].push_back({std::max(a, b), lv[k]});
            }
        }
        rev.reset();
        lv.reset();
        for (auto& kv : kept) {
            std::sort(kv.begin(), kv.end(),
                      [](const Kept& x, const Kept& y) { return x.j < y.j; });
        }
    } else {
#ifdef _OPENMP
        #pragma omp parallel for schedule(dynamic, 64) num_threads(n_cores) if(n_cores > 1)
