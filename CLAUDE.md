...
`compute_network(block_size = b)` calls `mr_block_network_cpp()` (`src/network_block.cpp`) on the standardised expression (`.standardise_for_cor()`, so `crossprod(zt)` is the `Rfast::cora()` correlation) and never forms the n x n matrix. Pass 1 computes correlation blocks and records each gene's cut-off correlation at a grid of row fractions f. Pass 2 recomputes the blocks, ranks each column exactly (`src/rank_column.h`), and keeps a pair as a candidate when both of its ranks are in the top fraction f. Validity rule: raw MR is `sqrt(R_ij * R_ji)` with both ranks at most n, so every pair with MR >= T has min rank >= T^2 / n; the candidate set is exact when `T^2 / n >= L`, the rank limit of fraction f, and otherwise f widens, up to all pairs. The thresholds are selected exactly over the candidates with the same `density_k()` (`src/density_k.h`) as `density_threshold_cpp()`. The result is `identical()` to the dense sparse build except at floating-point near-ties between correlations: dense uses `syrk`, blocks use `gemm`, and a last-bit difference can swap two ranks. Log MR's bound (`n^(2(1 - T))`) is looser, and small fixtures widen to f = 1, which saves nothing (a message says so). The candidates are held as 24-byte triplets, and at `store_density = 0.05` on 20 samples f = 0.2, so peak RSS is currently *above* dense (8.7 against 5.1 GB at n = 20,000). The default stays dense until the build is validated on Orion.
