...
`compute_network(block_size = b)` calls `mr_block_network_cpp()` (`src/network_block.cpp`) on the standardised expression (`.standardise_for_cor()`, so `crossprod(zt)` is the `Rfast::cora()` correlation) and never forms the n x n matrix. A single pass computes correlation blocks, ranks each column exactly (`src/rank_column.h`, self included, as the dense kernel does), and keeps per gene a list of (partner, rank) for partners in the top fraction f of that column (raw: `R >= n(1 - f)`; log: `R <= n f`), sorted by partner, 8 bytes an entry (ranks are multiples of 0.5, exact in a float up to n = 2^23). A pair is a candidate when each gene is on the other's list, found by binary search; its MR is computed exactly as the dense kernel does, and the thresholds are selected over the candidates with the same `density_k()` (`src/density_k.h`) as `density_threshold_cpp()`. Validity rule: raw MR is `sqrt(R_ij * R_ji)` with both ranks at most n, so every pair with MR >= T has min rank >= T^2 / n; the candidate set is exact when `T^2 / n >= n(1 - f)` (log: `n^(2(1 - T)) <= n f`), and otherwise the lists are rebuilt with f widened 1.5x, up to all pairs. f starts at `3 * store_density` (0.15 at the default; BDIS leaf needed about 0.11). The stored pairs come from one more sweep over the lists, which are then freed before the CSC slots are laid out. The result is `identical()` to the dense sparse build except at floating-point near-ties between correlations: dense uses `syrk`, blocks use `gemm`, and a last-bit difference can swap two ranks. Log MR's bound is looser, and small fixtures widen to f = 1, which saves nothing (a message says so). Measured on BDIS leaf (20 samples, n = 20,000, 8 threads, `dev/bench/network_memory.tsv`): peak RSS 1.2 GB (`block_size = 256`) / 1.4 GB (1024) in 6.8 / 7.4 s, against 5.1 GB in 8.2 s dense. The default stays dense until the build is validated on Orion.
