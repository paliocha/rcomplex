...
| `ortho_pairs.tsv` | Ortholog pairs (`Species1`, `Species2`, `hog`) in `compare_neighborhoods()` format: 320 one-to-one ortholog groups, plus 32 sp2 paralogs joining the first 4 groups of each module; 20 % (64) of the sp2 one-to-one partners are shuffled across modules (non-conserved orthologs). |
