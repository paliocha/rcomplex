...
# --- Tests for summarize_hog_comparison() ---

test_that("single-pair HOG reduces to -2*log(p)", {
  p_div1 <- 0.003
  p_div2 <- 0.005
  pair_p <- max(p_div1, p_div2)  # 0.005

  comparison <- data.frame(
    Species1 = "A_1", Species2 = "B_1", OrthoGroup = "HOG1",
    Species1.p.val.con = 0.99, Species2.p.val.con = 0.99,
    Species1.p.val.div = p_div1, Species2.p.val.div = p_div2,
    Species1.effect.size = 0.2, Species2.effect.size = 0.3,
    stringsAsFactors = FALSE
  )

  result <- summarize_hog_comparison(comparison, alternative = "less")

  expect_equal(nrow(result), 1)
  expect_equal(result$n_pairs, 1L)
  expect_equal(result$df, 2L)
  expect_equal(result$fisher_stat, -2 * log(pair_p), tolerance = 1e-10)
  # For k=1, Fisher chi-sq with df=2 gives same p as original
  expected_p <- stats::pchisq(-2 * log(pair_p), df = 2, lower.tail = FALSE)
  expect_equal(result$p.value, expected_p, tolerance = 1e-10)
  # For chi-sq(df=2): P(T > t) = exp(-t/2), and T = -2*log(p), so P = p
  expect_equal(result$p.value, pair_p, tolerance = 1e-10)
})

test_that("multi-pair HOG with all pairs diverged gives smaller p than any pair", {
  comparison <- data.frame(
    Species1 = paste0("A_", 1:4),
    Species2 = paste0("B_", 1:4),
    OrthoGroup = rep("HOG1", 4),
    Species1.p.val.con = rep(0.99, 4),
    Species2.p.val.con = rep(0.99, 4),
    Species1.p.val.div = c(0.01, 0.02, 0.005, 0.03),
    Species2.p.val.div = c(0.02, 0.01, 0.01, 0.02),
    Species1.effect.size = rep(0.3, 4),
    Species2.effect.size = rep(0.4, 4),
    stringsAsFactors = FALSE
  )
  ...
  expect_equal(result$df, 8L)

  # HOG p-value should be smaller than the best individual pair p-value
  pair_ps <- pmax(comparison$Species1.p.val.div, comparison$Species2.p.val.div)
  expect_true(result$p.value < min(pair_ps))
})

test_that("mixed HOG: one diverged pair + non-significant pairs gives intermediate p", {
  comparison <- data.frame(
    Species1 = paste0("A_", 1:3),
    Species2 = paste0("B_", 1:3),
    OrthoGroup = rep("HOG1", 3),
    Species1.p.val.con = rep(0.5, 3),
    Species2.p.val.con = rep(0.5, 3),
    Species1.p.val.div = c(0.001, 0.7, 0.8),
    Species2.p.val.div = c(0.002, 0.6, 0.9),
    Species1.effect.size = c(0.1, 1.0, 1.2),
    Species2.effect.size = c(0.15, 0.9, 1.1),
    stringsAsFactors = FALSE
  )

  result <- summarize_hog_comparison(comparison, alternative = "less")

  # The strong pair (p=0.002) is diluted by the two n.s. pairs
  # Combined p should be larger than the single strong pair's p
  expect_true(result$p.value > 0.002)
  # But smaller than the n.s. pairs
  expect_true(result$p.value < 0.9)
})

test_that("FDR correction is applied to HOG-level p-values", {
  comparison <- data.frame(
    Species1 = paste0("A_", 1:5),
    Species2 = paste0("B_", 1:5),
    OrthoGroup = paste0("HOG", 1:5),
    Species1.p.val.con = rep(0.5, 5),
    Species2.p.val.con = rep(0.5, 5),
    Species1.p.val.div = c(0.001, 0.01, 0.02, 0.5, 0.9),
    Species2.p.val.div = c(0.001, 0.01, 0.03, 0.6, 0.8),
    Species1.effect.size = rep(0.5, 5),
    Species2.effect.size = rep(0.5, 5),
    stringsAsFactors = FALSE
  )
  ...
  # Results should be ordered by p.value
  expect_true(all(diff(result$p.value) >= 0))
})

test_that("empty comparison handled by summarize_hog_comparison", {
  comparison <- data.frame(
    Species1 = character(0), Species2 = character(0),
    OrthoGroup = character(0),
    Species1.p.val.con = numeric(0), Species2.p.val.con = numeric(0),
    Species1.p.val.div = numeric(0), Species2.p.val.div = numeric(0),
    Species1.effect.size = numeric(0), Species2.effect.size = numeric(0),
    stringsAsFactors = FALSE
  )

  result <- summarize_hog_comparison(comparison, alternative = "less")
  expect_equal(nrow(result), 0)
  expect_named(result, c("OrthoGroup", "n_pairs", "fisher_stat", "mean_eff",
                          "min_eff", "df", "p.value", "q.value"))
})

test_that("alternative='greater' uses conservation p-values", {
  comparison <- data.frame(
    Species1 = paste0("A_", 1:2),
    Species2 = paste0("B_", 1:2),
    OrthoGroup = rep("HOG1", 2),
    Species1.p.val.con = c(0.001, 0.01),
    Species2.p.val.con = c(0.002, 0.005),
    Species1.p.val.div = c(0.99, 0.9),
    Species2.p.val.div = c(0.99, 0.95),
    Species1.effect.size = c(5, 3),
    Species2.effect.size = c(4, 2),
    stringsAsFactors = FALSE
  )

  result_con <- summarize_hog_comparison(comparison, alternative = "greater")
  result_div <- summarize_hog_comparison(comparison, alternative = "less")

  # Conservation should be highly significant (small p-values used)
  expect_true(result_con$p.value < 0.05)
  # Divergence should not be significant (large p-values used)
  expect_true(result_div$p.value > 0.5)
})

test_that("effect size is geometric mean of both directions", {
  comparison <- data.frame(
    Species1 = "A_1", Species2 = "B_1", OrthoGroup = "HOG1",
    Species1.p.val.con = 0.5, Species2.p.val.con = 0.5,
    Species1.p.val.div = 0.5, Species2.p.val.div = 0.5,
    Species1.effect.size = 4, Species2.effect.size = 9,
    stringsAsFactors = FALSE
  )

  result <- summarize_hog_comparison(comparison, alternative = "less")
  expect_equal(result$mean_eff, sqrt(4 * 9))  # = 6
  expect_equal(result$min_eff, sqrt(4 * 9))
})

test_that("multiple HOGs handled correctly", {
  comparison <- data.frame(
    Species1 = c("A_1", "A_2", "A_3"),
    Species2 = c("B_1", "B_2", "B_3"),
    OrthoGroup = c("HOG1", "HOG1", "HOG2"),
    Species1.p.val.con = c(0.5, 0.5, 0.5),
    Species2.p.val.con = c(0.5, 0.5, 0.5),
    Species1.p.val.div = c(0.001, 0.002, 0.5),
    Species2.p.val.div = c(0.001, 0.003, 0.6),
    Species1.effect.size = c(0.2, 0.3, 1.0),
    Species2.effect.size = c(0.3, 0.2, 1.0),
    stringsAsFactors = FALSE
  )
  ...
  # HOG1 has 2 pairs, HOG2 has 1
  hog1 <- result[result$OrthoGroup == "HOG1", ]
  hog2 <- result[result$OrthoGroup == "HOG2", ]
  expect_equal(hog1$n_pairs, 2L)
  expect_equal(hog2$n_pairs, 1L)
  # HOG1 should be more significant (strong divergence signal)
  expect_true(hog1$p.value < hog2$p.value)
})
